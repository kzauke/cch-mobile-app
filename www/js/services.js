...
.factory('Reviews', function ($http, $ionicUser) {
	...
	return {
		getRecentMealsForReviews: function () {
			...
			return $http.get(reviewDataSource, {
				cache: true
			});
		...
		submitReviewForMeal: function (rating, dnnMealId) {
			var data = $.param({MealID:dnnMealId,UserID:userid,StarRating:rating.StarRating,Comments:rating.Comments});
			var config = {
                headers : {
			...
			var submitReviewForMealURL = "http://chefnet.collegechefs.com/DesktopModules/DnnSharp/DnnApiEndpoint/Api.ashx?method=SubmitReview";
			
			
			$http.post(submitReviewForMealURL, data, config)
			...
			$http.get(submitReviewForMealURL).then(
				function successCallback(response) {
					console.log("success");
				},
				function errorCallback(response) {
