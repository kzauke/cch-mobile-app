...
.controller('MenusCtrl', function ($scope, Globals, Menus, $state, $ionicViewSwitcher, $stateParams, $ionicScrollDelegate, $location, $anchorScroll, $ionicPlatform, $ionicLoading, $ionicModal, $window, $timeout) {
	...
	$ionicModal.fromTemplateUrl('templates/modal-confirm-late-plate.html', {
		id: '1',					//arbitrary number to define different modals
		scope: $scope,
	...
	$ionicModal.fromTemplateUrl('templates/modal-cancel-late-plate.html', {
		id: '2',					//arbitrary number to define different modals
		scope: $scope,
		animation: 'slide-in-up'
	}).then(function (modal) {
		$scope.oModal2 = modal;
...
.controller('ReviewsCtrl', function ($scope, $stateParams, Globals, Reviews, Menus) {
	
	var noReviewedMealsMessage = "Looks like you haven't reviewed any meals yet";
	var getRecentMealsForReviews = Reviews.getRecentMealsForReviews();

	$scope.submitReviewForMeal = function(rating,dnnMealId) {
		return Reviews.submitReviewForMeal(rating,dnnMealId);
	};
	...
	getRecentMealsForReviews.then(
		...
		function (response) {
			...
			var recentMeals = [];
			var recentlyRatedMeals = [];
			var meals = response.data;
			
			for (var i = 0; i < meals.length; i++) {
				if (Menus.mealHasPassed(meals[i].name,meals[i].date)) {
					
					var d1 = new Date();
					d1.setHours(0, 0, 0, 0);
					d1.setDate(d1.getDate()-1); //set to yesterday

					var d2 = new Date(meals[i].date);
					d2.setHours(0, 0, 0, 0);
										
					//if meal was yesterday or today
					if (d2.getUTCDate() >= d1.getUTCDate()) {
						recentMeals.push(meals[i]);
					}
					
					//if meal was rated
					if (meals[i].StarRating !== "") {
						recentlyRatedMeals.push(meals[i]);
					}
					
					meals[i].date = d2;			//reset hours so it groups correctly
					
					$scope.recentlyRatedMeals = recentlyRatedMeals;
					$scope.recentMeals = recentMeals;
				}
				
				if (recentlyRatedMeals.length === 0) {
					$scope.noReviewedItems = noReviewedMealsMessage;
				}
